...
const failedWebSocketRoutes = new Set();
/**
 * Helper function to fetch data and update state
 */
export const fetchData = async (setData, fetchDataFunction) => {
    try {
 ...
 */
export const handleMessage = async (setData, fetchDataFunction) => {
    try {
 ...
 */
export const handleClose = (setData, ws, fetchDataFunction, sourceURL, timeoutRef, maxAttempts) => {
 ...
 */
const setupPolling = (setData, fetchDataFunction) => {
 ...
 */
export const connectWebSocket = (setData, ws, fetchDataFunction, sourceURL, timeoutRef, maxAttempts) => {
 ...
 */
export const useFetchWebSocket = (fetchDataFunction, sourceURL) => {
    const [data, setData] = useState([]);
    const ws = useRef(null);
    const timeoutRef = useRef(null);
    const maxAttemptsRef = useRef(3); // Reduced from 5 to 3 to fail faster
    ...
    useEffect(() => {
        ...
        if (webSocketManager) {
            ...
            if (webSocketManager.status === CONNECTION_STATES.AUTHENTICATED) {
                const unsubscribe = webSocketManager.subscribe(sourceURL, (wsData) => {
        ...
        if (sourceURL && shouldUseWebSockets) {
            // Connect WebSocket
            const cleanup = connectWebSocket(setData, ws, fetchDataFunction, sourceURL, timeoutRef, maxAttemptsRef);

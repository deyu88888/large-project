...
export function PrivateGuard({ children, requiredRole }) {
    ...
    if (!authState.isAuthorized) {
        ...
        return isPublicRoute ? _jsx(_Fragment, { children: children }) : _jsx(Navigate, { to: "/login", replace: true });
    }
    ...
    if (authState.isAuthorized && isPublicRoute) {
        ...
        return _jsx(Navigate, { to: user?.role === "admin" ? "/admin" : "/student", replace: true });
    }
    ...
    if (requiredRole && user?.role !== requiredRole) {
        ...
        return _jsx(Navigate, { to: `/${user?.role || 'student'}`, replace: true });
    }

...
vi.mock("react-big-calendar", async () => {
    ...
    return {
        ...
        Calendar: vi.fn((props) => {
            const { events = [] } = props;
            return (_jsx("div", { "data-testid": "mock-calendar", children: events.map((evt, idx) => (_jsx("div", { "data-testid": "mock-event", children: evt.title }, idx))) }));
        }),
    };
});
...
describe("EventCalendar (Vitest) - Partial RBC Mock", () => {
    ...
    it("renders the mocked Calendar and keeps the real momentLocalizer", () => {
        ...
        render(_jsx(EventCalendar, { events: events }));
    ...
    it("applies the custom style logic for events (eventPropGetter)", () => {
        ...
        const zeroStyle = eventPropGetter(zeroEvent, zeroEvent.start, zeroEvent.end, false);
        ...
        const normalStyle = eventPropGetter(normalEvent, normalEvent.start, normalEvent.end, false);
    ...
    it("uses the custom event component (CustomEvent) to display title and times", () => {
        ...
        const { container } = render(_jsx(CustomEventComponent, { event: events[0] }));

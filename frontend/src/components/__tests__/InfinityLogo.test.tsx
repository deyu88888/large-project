...
describe('InfinityLogo Component', () => {
  it('renders the SVG with correct attributes', () => {
    const { container } = render(<InfinityLogo />);
  ...
  it('has correct MUI Box component styles', () => {
    const { container } = render(<InfinityLogo />);
    const svgElement = container.querySelector('svg');
    expect(svgElement).toBeTruthy();
    expect(svgElement?.classList.toString()).toMatch(/MuiBox-root/);
    const computedStyle = window.getComputedStyle(svgElement!);
    expect(computedStyle.width).toBe('32px');
    expect(computedStyle.height).toBe('32px');
  });

  it('renders the correct path for the infinity symbol', () => {
    const { container } = render(<InfinityLogo />);
  ...
  it('fills with current color', () => {
    ...
    const svgElement = container.querySelector('svg');
    const computedStyle = window.getComputedStyle(svgElement!);
    expect(computedStyle.fill).toBe('currentColor');
  });
});
...
describe('ActivityLogList Component', () => {
    ...
    const renderComponent = (searchTerm = '') => {
        return render(_jsx(ThemeProvider, { theme: lightTheme, children: _jsx(SearchContext.Provider, { value: { searchTerm, setSearchTerm: vi.fn() }, children: _jsx(ActivityLogList, {}) }) }));
    };
    it('renders loading state initially', async () => {
        // Mock delay for the API response
        apiClient.get.mockImplementation(() => new Promise(resolve => setTimeout(() => resolve({ data: mockActivityLogs }), 100)));
        renderComponent();
    ...
    it('renders correctly in dark mode', async () => {
        render(_jsx(ThemeProvider, { theme: darkTheme, children: _jsx(SearchContext.Provider, { value: { searchTerm: '', setSearchTerm: vi.fn() }, children: _jsx(ActivityLogList, {}) }) }));
        await waitFor(() => {
            expect(screen.queryByRole('progressbar')).not.toBeInTheDocument();
        });

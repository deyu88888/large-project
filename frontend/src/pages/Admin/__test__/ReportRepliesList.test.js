...
vi.mock('@mui/x-data-grid', () => ({
    DataGrid: ({ rows, columns }) => (_jsxs("div", { "data-testid": "data-grid", children: [_jsx("div", { "data-testid": "row-count", children: rows.length }), _jsx("div", { "data-testid": "column-count", children: columns.length }), rows.map((row) => (_jsxs("div", { "data-testid": `row-${row.id}`, children: [row.subject, _jsx("div", { "data-testid": `view-thread-${row.id}`, children: _jsx("button", { onClick: () => columns.find(c => c.field === 'actions')?.renderCell({ row }) }) })] }, row.id)))] })),
...
describe('ReportRepliesList', () => {
    ...
    it('renders loading state initially', async () => {
        ...
        render(_jsx(SearchContext.Provider, { value: { searchTerm: '', setSearchTerm: vi.fn() }, children: _jsx(MemoryRouter, { children: _jsx(ReportRepliesList, {}) }) }));
    ...
    it('adjusts layout based on drawer state', async () => {
        ...
        vi.mocked(fetchReportsWithReplies).mockResolvedValue(mockReports);
        const { container } = render(_jsx(SearchContext.Provider, { value: { searchTerm: '', setSearchTerm: vi.fn() }, children: _jsx(MemoryRouter, { children: _jsx(ReportRepliesList, {}) }) }));

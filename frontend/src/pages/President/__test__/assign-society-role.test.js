...
describe('AssignSocietyRole Component', () => {
    ...
    const renderComponent = (themeModeOverride = 'light') => {
        ...
        return render(_jsx(ThemeProvider, { theme: theme, children: _jsx(MemoryRouter, { initialEntries: [`/president/assign-role/${mockSocietyId}/${mockStudentId}`], children: _jsx(Routes, { children: _jsx(Route, { path: "/president/assign-role/:society_id/:student_id", element: _jsx(AssignSocietyRole, {}) }) }) }) }));
    };
    ...
    it('calls API with correct payload when assigning a role', async () => {
        ...
        await waitFor(() => {
            expect(apiClient.patch).toHaveBeenCalledWith(`/api/manage-society-details/${mockSocietyId}`, { vice_president: Number(mockStudentId) });
        });
    ...
    it('handles different role assignments with correct payloads', async () => {
        ...
        await waitFor(() => {
            expect(apiClient.patch).toHaveBeenCalledWith(`/api/manage-society-details/${mockSocietyId}`, { event_manager: Number(mockStudentId) });
        });
        vi.clearAllMocks();
        ...
        await waitFor(() => {
            expect(apiClient.patch).toHaveBeenCalledWith(`/api/manage-society-details/${mockSocietyId}`, { treasurer: Number(mockStudentId) });
        });
    });
    it('handles role assignment error', async () => {
        const errorMessage = 'Failed to assign role';
        apiClient.patch.mockRejectedValueOnce(new Error(errorMessage));
        renderComponent();
    ...
    it('disables buttons and shows loading spinner during role assignment', async () => {
        apiClient.patch.mockImplementationOnce(() => new Promise(() => { }));
        renderComponent();
        const eventManagerButton = screen.getByText('Event Manager');
        fireEvent.click(eventManagerButton);
    ...
    it('handles loading state toggling correctly', async () => {
        ...
        apiClient.patch.mockImplementationOnce(() => new Promise(resolve => setTimeout(() => resolve({ data: { success: true } }), 100)));

...
describe('ReportToAdmin Component', () => {
    ...
    const renderComponent = async (theme = lightTheme) => {
        let rendered;
        await act(async () => {
            rendered = render(_jsx(ThemeProvider, { theme: theme, children: _jsx(MemoryRouter, { initialEntries: ['/president/report-to-admin'], children: _jsx(Routes, { children: _jsx(Route, { path: "/president/report-to-admin", element: _jsx(ReportToAdmin, {}) }) }) }) }));
    ...
    it('handles submission error and logs to console', async () => {
        ...
        apiClient.post.mockRejectedValueOnce(testError);
        await renderComponent();
        const subjectInput = screen.getByRole('textbox', { name: /Subject/i });
        const detailsInput = screen.getByRole('textbox', { name: /Report Details/i });
        const submitButton = screen.getByRole('button', { name: /submit report/i });

...
describe('PresidentPage Component', () => {
    ...
    function renderComponent() {
        return render(_jsx(ThemeProvider, { theme: theme, children: _jsx(MemoryRouter, { initialEntries: ['/president-page/123'], children: _jsx(Routes, { children: _jsx(Route, { path: "/president-page/:society_id", element: _jsx(PresidentPage, {}) }) }) }) }));
    }
    ...
    it('handles error when no society ID is available', async () => {
        ...
        renderComponent();
        await waitFor(() => {
            expect(consoleErrorSpy).toHaveBeenCalledWith('Error fetching president data:', expect.objectContaining({ message: 'No society ID available' }));
        });
        consoleErrorSpy.mockRestore();
    });

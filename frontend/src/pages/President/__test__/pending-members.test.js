...
describe('PendingMembers Component', () => {
    ...
    function renderComponent() {
        return render(_jsx(MemoryRouter, { children: _jsx(PendingMembers, {}) }));
    }
    ...
    it('handles API error when fetching pending members', async () => {
        ...
        renderComponent();
        await waitFor(() => {
            expect(consoleErrorSpy).toHaveBeenCalledWith('Error fetching pending members:', expect.any(Error));
        });
    ...
    it('handles error when updating member status', async () => {
        ...
        fireEvent.click(acceptButtons[0]);
        await waitFor(() => {
            expect(consoleErrorSpy).toHaveBeenCalledWith('Error updating member status:', expect.any(Error));
        });
        consoleErrorSpy.mockRestore();
    });

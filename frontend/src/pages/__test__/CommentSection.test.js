...
vi.mock('../../components/CommentItem', () => ({
    CommentItem: ({ comment, onReply }) => (_jsxs("div", { "data-testid": `comment-${comment.id}`, children: [_jsx("div", { children: comment.content }), _jsxs("div", { children: ["by ", comment.user_data.username] }), _jsx("button", { onClick: () => onReply(comment.id, 'Test reply'), "data-testid": `reply-button-${comment.id}`, children: "Reply" })] })),
}));
...
describe('CommentSection Component', () => {
    ...
    it('renders loading state initially', async () => {
        ...
        render(_jsx(ThemeProvider, { theme: theme, children: _jsx(CommentSection, { eventId: mockEventId }) }));
    ...
    it('handles adding a new comment', async () => {
        ...
        expect(apiClient.post).toHaveBeenCalledWith("/api/comments/?event_id=${eventId}", expect.objectContaining({
            event: mockEventId,
    ...
    it('handles API error when replying to a comment', async () => {
        ...
        expect(consoleErrorSpy).toHaveBeenCalledWith('Error replying comment:', expect.any(Error));
        consoleErrorSpy.mockRestore();
    });
});

...
describe('PresidentDrawer Component', () => {
    ...
    const setup = async (drawerOpen = true) => {
        let renderResult;
        await act(async () => {
            renderResult = render(_jsx(MemoryRouter, { children: _jsx(PresidentDrawer, { drawer: drawerOpen, toggleDrawer: mockToggleDrawer, location: mockLocation }) }));
    ...
    it('navigates to the join society page', async () => {
        ...
        const joinSocietiesLink = joinSocietiesElements.find(element => element.closest('a') !== null).closest('a');

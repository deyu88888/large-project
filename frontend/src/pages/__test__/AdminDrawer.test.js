...
describe('AdminDrawer Component', () => {
    ...
    const setup = async (drawerOpen = true, isSuperAdmin = false) => {
        ...
        let renderResult;
        await act(async () => {
            renderResult = render(_jsx(MemoryRouter, { children: _jsx(AdminDrawer, { drawer: drawerOpen, toggleDrawer: mockToggleDrawer, location: mockLocation }) }));
        });
        return renderResult;
    };
    ...
    it('renders correct admin role label based on user type', async () => {
        ...
        const adminLabel = screen.getAllByText('Super Admin').find(element => element.tagName.toLowerCase() === 'p');

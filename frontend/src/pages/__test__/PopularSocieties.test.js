...
describe('PopularSocieties Component', () => {
    ...
    it('renders loading state initially', async () => {
        ...
        getPopularSocieties.mockImplementation(() => new Promise((resolve) => setTimeout(() => resolve(mockSocieties), 100)));
        render(_jsx(ThemeProvider, { theme: lightTheme, children: _jsx(PopularSocieties, {}) }));
    ...
    it('renders correctly in dark mode', async () => {
        // Mock successful API response
        getPopularSocieties.mockResolvedValue(mockSocieties);
        render(_jsx(ThemeProvider, { theme: darkTheme, children: _jsx(PopularSocieties, {}) }));
        // Wait for the component to load data
        await waitFor(() => {
            expect(screen.getByText('Most Popular Societies')).toBeInTheDocument();
        });

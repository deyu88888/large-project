...
import { SnackbarState } from "../types/snackbar/snackbarState";
...
const Support = () => {
  ...
  const handleIssueTypeChange = (event: SelectChangeEvent<string>) => {
  ...
  const submitReport = async () => {
    const reportData = { subject: issueType, details: message, email };
      ...
      {faqSections.map((section, sectionIndex) => (
        <Box key={`section-${sectionIndex}`} mb={4}>
          ...
          {section.questions.map((faq, faqIndex) => {
            const panelId = `panel-${sectionIndex}-${faqIndex}`;
            return (
              <Accordion
                key={panelId}
                ...
                sx={{
                  ...
                  color: colors.grey[100],
                  marginBottom: "0.5rem",
                  borderRadius: expanded === panelId ? "8px" : "8px !important",
                  '&:before': { display: 'none' },
                  boxShadow: isLight
              ...
              >
                ...
                <AccordionDetails sx={{ paddingTop: 0 }}>
                  <Typography sx={{ color: colors.grey[200] }}>{faq.answer}</Typography>
                </AccordionDetails>
              </Accordion>
            );
          })}
    ...
    >
      <Box p={3}>
        <Box display="flex" alignItems="center" mb={3}>
          <ReportProblemIcon sx={{ color: colors.blueAccent[400], mr: 1, fontSize: "2rem" }} />
          <Typography variant="h3" sx={{ color: colors.grey[100], fontWeight: 600 }}>
            Report an Issue
          </Typography>
        </Box>
          ...
          <Box mb={3}>
            <FormControl fullWidth required variant="outlined">
              ...
              <Select
                ...
                label="Issue Type"
                sx={{ backgroundColor: isLight ? colors.primary[900] : colors.primary[600] }}
      ...
      >
        <Box display="flex" alignItems="center" mb={2}>
          <EmailIcon sx={{ color: colors.blueAccent[400], mr: 1 }} />
          <Typography variant="h6" sx={{ color: colors.grey[100] }}>
            Email Support Directly
          </Typography>
        </Box>
        <Typography sx={{ color: colors.grey[200], mb: 2 }}>
    ...
    >
      <Container maxWidth="lg">
        <Box mb={5} textAlign="center">
          <Typography variant="h2" component="h1" sx={{ color: colors.grey[100], fontWeight: 700, mb: 2 }}>
            Support Center
          </Typography>
          <Typography variant="h5" sx={{ color: colors.grey[200], maxWidth: 700, mx: "auto" }}>

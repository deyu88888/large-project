...
describe("ViewEvents", () => {
    ...
    const renderComponent = (theme = lightTheme) => render(_jsx(ThemeProvider, { theme: theme, children: _jsx(MemoryRouter, { children: _jsx(MyJoinedEvents, {}) }) }));
    ...
    it('renders "No upcoming events." when there are no events in light theme', async () => {
        apiClient.get.mockImplementationOnce((url) => {
            if (url === "/api/events") {
                return Promise.resolve({ data: [] });
            }
            return Promise.resolve({ data: [] });
    ...
    it("logs an error if fetching events fails", async () => {
        ...
        apiClient.get.mockImplementationOnce(() => Promise.reject(new Error("Fetch failed")));
        renderComponent();
        await waitFor(() => expect(screen.getByText(/All Events/i)).toBeInTheDocument());
        expect(screen.getByText(/No upcoming events./i)).toBeInTheDocument();

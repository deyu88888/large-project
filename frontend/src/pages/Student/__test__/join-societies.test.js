...
describe('JoinSocieties Page', () => {
    beforeEach(() => {
        ...
        getRecommendedSocieties.mockResolvedValue([
            {
                society: {
                    id: 1,
    ...
    const renderComponent = () => render(_jsx(ThemeProvider, { theme: createTheme(), children: _jsx(MemoryRouter, { children: _jsx(JoinSocieties, {}) }) }));
    ...
    it('renders empty state when no societies are returned', async () => {
        getRecommendedSocieties.mockResolvedValueOnce([]);
        renderComponent();
    ...
    it('handles error when fetching society recommendations fails', async () => {
        getRecommendedSocieties.mockRejectedValueOnce(new Error('Fetch error'));

...
describe('StartSociety', () => {
    ...
    const renderComponent = (theme = lightTheme) => render(_jsx(ThemeProvider, { theme: theme, children: _jsx(MemoryRouter, { children: _jsx(StartSociety, {}) }) }));
    ...
    it('shows an error when only society name is provided', async () => {
        renderComponent();
        const nameInput = screen.getByLabelText(/Society Name/i);
        await act(async () => {
    ...
    it('submits the form successfully and clears fields on a 201 response', async () => {
        ...
        await waitFor(() => expect(screen.getByText(/Society creation request submitted successfully!/i)).toBeInTheDocument());
    ...
    it('shows an error message when axios.post fails', async () => {
        ...
        await waitFor(() => expect(screen.getByText(/Failed to create society. Please try again later./i)).toBeInTheDocument());
    });
    ...
    it('handles axios error with response data correctly', async () => {
        ...
        axios.post.mockRejectedValueOnce(errorWithResponse);
        renderComponent();
        const nameInput = screen.getByLabelText(/Society Name/i);
        const descriptionInput = screen.getByLabelText(/Description/i);
        await act(async () => {
    ...
    it('handles different types of axios errors correctly', async () => {
        ...
        const consoleSpy = vi.spyOn(console, 'error');
        axios.post.mockRejectedValueOnce(errorWithStatusCode);
        renderComponent();
        const nameInput = screen.getByLabelText(/Society Name/i);
        const descriptionInput = screen.getByLabelText(/Description/i);
        await act(async () => {

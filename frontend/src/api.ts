...
export const apiPaths = {
  USER: {
    ...
    ADMINSTUDENTVIEW: (studentId: number) =>
      `/api/admin-manage-student-details/${studentId}`,
    ADMINSOCIETYVIEW: (societyId: number) =>
      `/api/admin-manage-society-details/${societyId}`, // admin society view
    ADMINEVENTVIEW: (eventId: number) =>
      `/api/admin-manage-event-details/${eventId}`,
    ADMINVIEW: (adminId: number) =>
      `/api/admin-manage-admin-details/${adminId}`,
    DELETE: (targetType: string, targetId: number) =>
      `/api/delete/${targetType}/${targetId}`,
  ...
  SOCIETY: {
    ...
    RECOMMENDATION_FEEDBACK_ANALYTICS:
      "/api/recommendation-feedback/analytics/",
  },
  EVENTS: {

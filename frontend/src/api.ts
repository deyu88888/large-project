...
/** For retrieving detail about a single news post. */
export interface NewsPostDetail {
  ...
  is_pinned: boolean;
  tags: string[];
  // ... Add other fields from SocietyNews if needed (author, image, etc.)
}
...
export interface NewsCommentData {
  ...
  likes: number[];
  // ... Add additional fields if your NewsCommentSerializer returns them
}

/** Payload to create or update a comment. */
export interface CommentPayload {
  content: string;
  parent_comment?: number | null;
}

/** Toggles a comment's like status and returns updated info. */
...
/**
 * Fetch top-level comments for a specific news post.
 */
...
/**
 * Create a new comment on a specific news post.
 */
 ...
 * Returns the action taken ("liked"/"unliked") and updated total likes.
 */

...
class SocietyShowreelRequestModelTest(TransactionTestCase):
        ...
        try:
            if self.showreel_request.photo:
                file_path = os.path.join(
                    settings.MEDIA_ROOT, self.showreel_request.photo.name)
    ...
    def test_society_relationship(self):
        ...
        self.assertEqual(
            self.society_request.showreel_images_request.count(), 1)
        self.assertEqual(
            self.society_request.showreel_images_request.first(), self.showreel_request)
        ...
        self.assertEqual(
            self.society_request.showreel_images_request.count(), 2)
        self.assertIn(second_showreel,
                      self.society_request.showreel_images_request.all())
    ...
    def test_cascade_deletion(self):
        ...
        self.assertEqual(
            SocietyShowreelRequest.objects.filter(
                society_id=society_request_id).count(),
            0
        )
    ...
    def test_upload_path(self):
        ...
        self.assertTrue(self.showreel_request.photo.name.startswith(
            'society_showreel_request/'))
    ...
    def test_multiple_images_for_society_request(self):
        ...
        self.assertEqual(
            self.society_request.showreel_images_request.count(), 6)

        captions = [
            req.caption for req in self.society_request.showreel_images_request.all()]

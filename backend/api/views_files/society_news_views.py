...
class SocietyNewsListView(APIView):
    ...
    def get(self, request, society_id):
        ...
        try:
            ...
            if hasattr(request.user, 'student'):
                has_management = has_society_management_permission(
                    request.user.student, society)
...
class SocietyNewsDetailView(APIView):
    ...
    def get(self, request, news_id):
        ...
        serializer = SocietyNewsSerializer(
            news_post, context={'request': request})
        return Response(serializer.data, status=status.HTTP_200_OK)
    ...
    def put(self, request, news_id):
        ...
        if current_status == "Published" or current_status == "Rejected":
            ...
            resubmit_rejected = current_status == "Rejected" and data.get(
                'resubmit', 'false').lower() == 'true'
            ...
            if content_changed or files_present or resubmit_rejected:
                print(
                    f"Content changed: {content_changed}, Files present: {files_present}, Resubmit: {resubmit_rejected}")
                ...
                serializer = SocietyNewsSerializer(
                    news_post, data=data, partial=True, context={'request': request})
                if serializer.is_valid():
                    ...
                    if latest_approved and latest_rejected:
                        if latest_approved.reviewed_at > latest_rejected.reviewed_at:
                            request_data[
                                'admin_notes'] = f"Revision of previously approved content (approved on {latest_approved.reviewed_at.strftime('%Y-%m-%d %H:%M')})"
                        else:
                            request_data[
                                'admin_notes'] = f"Revision of previously rejected content (rejected on {latest_rejected.reviewed_at.strftime('%Y-%m-%d %H:%M')})"
                    ...
                    publication_request = NewsPublicationRequest.objects.create(
                        **request_data)
                    print(
                        f"Created new publication request: {publication_request.id} for news post: {news_post.id}")

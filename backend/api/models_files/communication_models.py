...
class ReportReply(models.Model):
    ...
    report = models.ForeignKey(
        "api.AdminReportRequest", on_delete=models.CASCADE, related_name='replies')
    parent_reply = models.ForeignKey(
        'self', on_delete=models.CASCADE, null=True, blank=True, related_name='child_replies')
    content = models.TextField(blank=False)
    created_at = models.DateTimeField(auto_now_add=True)
    replied_by = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name='report_replies')
    # To distinguish between admin and president replies
    is_admin_reply = models.BooleanField(default=False)
    read_by_students = models.ManyToManyField(
        User, related_name='read_report_replies', blank=True)
    hidden_for_students = models.ManyToManyField(
        User, related_name='hidden_replies', blank=True)
...
class NewsNotification(models.Model):
    ...
    target_society = models.ForeignKey(
        "api.Society", on_delete=models.CASCADE, null=True, blank=True, related_name="news_notifications")
    target_event = models.ForeignKey(
        "api.Event", on_delete=models.CASCADE, null=True, blank=True, related_name="event_notifications")
    target_multiple_societies = models.ManyToManyField(
        "api.Society", blank=True, related_name="multi_society_news")
...
class BroadcastMessage(models.Model):
    ...
    """
    sender = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="sent_broadcasts")
    societies = models.ManyToManyField(
        "api.Society", related_name="broadcasts", blank=True)
    events = models.ManyToManyField(
        "api.Event", related_name="broadcasts", blank=True)
...
class SocietyNews(models.Model):
    ...
    image = models.ImageField(upload_to="society_news/images/",
                              blank=True, null=True, default="news_icons/newspaper.jpg")
    attachment = models.FileField(
        upload_to="society_news/attachments/", blank=True, null=True)
    ...
    def increment_view_count(self, amount: int = 1):
        ...
        """
        SocietyNews.objects.filter(pk=self.pk).update(
            view_count=F('view_count') + amount)
...
class NewsPublicationRequest(models.Model):
    ...
    admin_notes = models.TextField(
        blank=True, null=True, help_text="Feedback or notes from the admin, especially for rejections")
